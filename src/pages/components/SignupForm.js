...
const SignupForm = (props) => {
  ...
  const getCSRF = async () => {
    ...
    const inscription = async (e) => {
      ...
      {
        if($('.radioBtn')[i].classList.contains('inputform'))
          sexe = $('.radioBtn')[i].value
      }
      
      console.log('sexe',sexe)
      let prenom = $('#in1')[0].value
      let nom = $('#in2')[0].value
      let email = $('#in3')[0].value
      let mdp = $('#in4')[0].value
      let mdp2 = $('#in5')[0].value
      const regex = /^(([^<>()[\]\.,;:\s@\"]+(\.[^<>()[\]\.,;:\s@\"]+)*)|(\".+\"))@(([^<>()[\]\.,;:\s@\"]+\.)+[^<>()[\]\.,;:\s@\"]{2,})$/i;
      console.log('valeuuuuuuuuuuuuuuuurs:',sexe,prenom,nom)
      if(sexe==" "){
        setError('Veuillez nous indiquer un sexe')
        return ('sexe erreur')
      }
      if(prenom==" "){
        setError('Veuillez nous indiquer un prénom')
        return ('prenom erreur')
      }
      if(nom==" "){
        setError('Veuillez nous indiquer un nom')
        return ('nom erreur')
      }
      if(email==" "){
        setError('Veuillez nous indiquer un email')
        return ('email erreur')
      }
      if(!regex.test(email)){
      ...
      if(inputs[$(".inputform")[0].name!='sexe'])
        return setError("Selectionner un sexe")
      var inputsForm = JSON.stringify(inputs);
      console.log(inputsForm)
      let r = await getCSRF()
      ...
      var url2 = "http://localhost:8000/signup/"
    ...
    return (
        <div className="d-flex flex-column align-items-center justify-content-center p-3">
          ...
          <div className="d-flex justify-content-center align-items-center w-100 p-2">
            <input type="radio" className="radioBtn" value="F" name="sexe" 
            onClick={(e)=>{
              $(".radioBtn").removeClass("inputform")
              e.target.classList.add("inputform")              
              }}/>
            <input type="radio" className="radioBtn" value="H" name="sexe" 
            onClick={(e)=>{
              $(".radioBtn").removeClass("inputform")
              e.target.classList.add("inputform")              
              }} />
            <input type="radio" className="radioBtn" value="O" name="sexe" 
            onClick={(e)=>{
              $(".radioBtn").removeClass("inputform")
              e.target.classList.add("inputform")              
              }} />
          ...
          <div className="d-flex justify-content-between align-items-center w-100">
            <div className="formrow me-2">
              <input id="in1" type="text" name='prenom' className="inputform" placeholder=" " required/>
              <label for="in1" className="labelform">Prénom*</label> 
            </div>
            <div className="formrow ms-2">
              <input id="in2" type="text" name='nom' className="inputform" placeholder=" " required/>
          ...
          <div className="formrow w-100">
            <input id="in6" type="text" name='adresse' className="inputform" autoComplete="off" placeholder=" " required/>
            <label for="in6" className="labelform">Adresse*</label> 
          </div>
            
          <div className="d-flex justify-content-between align-items-center w-100">
            <div className="formrow ">
              <input id="in7" type="text" name='ville' className="inputform " placeholder=" "  required/>
              <label for="in7" className="labelform">Ville*</label> 
            </div>
            <div className="formrow ">
              <input id="in8" type="text" name='cp' className="inputform " placeholder=" "  required/>
          ...
          <div className="d-flex justify-content-between align-items-center w-100">
            <div className="formrow ">
              <input id="in9" type="text" name='pays' className="inputform " placeholder=" " required/>
              <label for="in9" className="labelform">Pays*</label> 
            </div>
            <div className="formrow ">
              <input id="in10" type="text" name='telephone' className="inputform " placeholder=" " />
          ...
          <div className="w-100 d-flex justify-content-between align-items-center pb-3">
            <div className="d-flex align-items-center ">
              <input type="checkbox" className="formCheckbox me-2" required />
              <label for="stayco" className="m-0">
                Accepter les conditions
              </label>
            </div>
            <a href="" className="link">CGV et confidentialité</a>
          </div>
          <p className="text-danger">
              {error}
          </p>
          <button className="formbtn" onClick={(e) => inscription(e)}>
          S'inscrire
          </button>
        <div className="traitHorizontal my-5">
          ...
          <a className="googleBtn" onClick={() => props.setModalcontent(<Loginform s setModalcontent={props.setModalcontent}  showHideModal={props.showHideModal} user={props.user} token={props.token} setToken={props.setToken} logged_in={props.logged_in} setLogin={props.setLogin} setPrenom={props.setPrenom} />)}>
          Se connecter
          </a>
        </form>
        </div>
